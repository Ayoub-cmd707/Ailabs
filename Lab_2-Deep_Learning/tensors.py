...
def lin_layer_forward(weights: torch.Tensor, random_image: torch.Tensor) -> torch.Tensor:
    ...
    product = weights*random_image
    output = torch.sum(product)
    return output
...
def tensor_network():
    ...
    # use tensor operations, recall the following formula we've seen during class: x <- x - alpha * (x') ->grad weights

    weights = torch.subtract(weights,(0.5 * weights.grad))
